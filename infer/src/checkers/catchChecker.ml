...
module Domain = struct (* implementation of join, widen, <=, pp *)
  ...
  let join astate1 astate2 =
    ...
    then { astate with in_catch = true; seen_call = astate1.seen_call || astate2.seen_call }
    else { astate with in_catch = false; seen_call = astate1.seen_call || astate2.seen_call }
  let widen ~prev ~next ~num_iters:_ = join prev next
  let pp fmt (astate1) = function
    F.printf fmt "(State: in_catch=%b, seen_call=%b)" (astate1.in_catch, astate1.seen_call)
end
...
module TransferFunctions (CFG : ProcCfg.S) = struct
  let exec_instr astate proc_data node instr =
    ...
    if is_start_of_catch then {astate with in_catch=true;} else {astate with in_catch=astate.in_catch || false}
end
