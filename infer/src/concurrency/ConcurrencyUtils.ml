...
let get_java_constructor_summaries_of {InterproceduralAnalysis.proc_desc; tenv; analyze_dependency}
  ...
  in
  let methods = List.map ~f:(fun ({name} : Struct.tenv_method) -> name) methods in
  (* keep only the constructors *)
  let res =
    List.filter
      ~f:Procname.(function Java jname -> Java.is_constructor jname | _ -> false)
      methods
  in
  (* get the summaries of the constructors *)
  List.filter_map ~f:(fun proc_name -> analyze_dependency proc_name |> AnalysisResult.to_option) res
