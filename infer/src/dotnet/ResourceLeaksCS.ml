...
module TransferFunctions (CFG : ProcCfg.S) = struct
  ...
  let exec_instr (astate : ResourceLeakCSDomain.t)
      {InterproceduralAnalysis.proc_desc= proc_desc; tenv; analyze_dependency; _} _ (instr : HilInstr.t) =
      ...
      let concat_types = 
        Hashtbl.iter (fun x y -> 
          if ResourceLeakCSDomain.check_count x post then
            leak_list := ResourceLeakCSDomain.LeakList.append_one !leak_list y) !type_map
      in
      concat_types ;
...
let checker ({InterproceduralAnalysis.proc_desc} as analysis_data) =
  let result = Analyzer.compute_post analysis_data ~initial:ResourceLeakCSDomain.initial proc_desc in
