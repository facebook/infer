...
let[@inline] filter_map seq ~f k = seq (fun x -> match f x with None -> () | Some y -> k y)
...
let keep_ok seq k = seq (function Result.Ok x -> k x | Result.Error _ -> ())

let keep_error seq k = seq (function Result.Error x -> k x | Result.Ok _ -> ())
...
module MList = struct
  ...
  let length l =
    let rec len acc l = match l with Nil -> acc | Cons (_, n, tl) -> len (acc + !n) !tl in
    len 0 l
...
let sorted seq ~cmp =
  ...
  try
    seq (fun x ->
        match !prev with
        | Some y when cmp y x > 0 ->
            raise_notrace Exit_sorted
        | _ ->
            prev := Some x ) ;
    true
...
let join s1 s2 ~f:join_row k =
  s1 (fun a -> s2 (fun b -> match join_row a b with None -> () | Some c -> k c))
...
let join_by (type a) c1 c2 ~key1:f1 ~key2:f2 ~eq ~hash ~f:merge =
  ...
  c2 (fun y ->
      ...
      List.iter (fun x -> match merge key x y with None -> () | Some z -> res := z :: !res) xs ) ;
  fun yield -> List.iter yield !res


type ('a, 'b) join_all_cell = {mutable ja_left: 'a list; mutable ja_right: 'b list}
...
let join_all_by (type a) c1 c2 ~key1:f1 ~key2:f2 ~eq ~hash ~f:merge =
  ...
  Tbl.iter
    (fun key cell ->
      match merge key cell.ja_left cell.ja_right with None -> () | Some z -> res := z :: !res )
    tbl ;
  fun yield -> List.iter yield !res
...
let max seq ~lt =
  let ret = ref None in
  seq (fun x -> match !ret with None -> ret := Some x | Some y -> if lt y x then ret := Some x) ;
  !ret


let max_exn seq ~lt = match max ~lt seq with Some x -> x | None -> raise_notrace Not_found

let min seq ~lt =
  let ret = ref None in
  seq (fun x -> match !ret with None -> ret := Some x | Some y -> if lt x y then ret := Some x) ;
  !ret


let min_exn seq ~lt = match min ~lt seq with Some x -> x | None -> raise Not_found
...
let head_exn seq = match head seq with None -> invalid_arg "Iter.head_exn" | Some x -> x
...
let take_while seq ~f k =
  try seq (fun x -> if f x then k x else raise_notrace ExitTakeWhile) with ExitTakeWhile -> ()
...
let fold_opt seq s ~f =
  ...
  try
    seq (fun x ->
        match f x !state with Some s -> state := s | None -> raise_notrace Stop_fold_opt ) ;
    Some !state
...
let fold_result (type s e) seq s ~f =
  ...
  try
    seq (fun x ->
        match f x !state with Ok s -> state := s | Error _ as e -> raise_notrace (Stop e) ) ;
    Ok !state
  with Stop e -> e
...
let fold_until (type res) seq s ~f ~finish =
  ...
  try
    seq (fun x ->
        match f x !state with `Continue s -> state := s | `Stop r -> raise_notrace (Stop r) ) ;
    finish !state
  with Stop r -> r
...
let pop seq = match head seq with Some x -> Some (x, drop 1 seq) | None -> None

let pop_exn seq = (head_exn seq, drop 1 seq)

let reduce seq ~f = match pop seq with Some (x, seq) -> Some (fold seq x ~f) | None -> None
...
let findi seq ~f = find_mapi ~f:(fun i x -> if f i x then Some x else None) seq

let find_exn seq ~f = match find ~f seq with Some x -> x | None -> raise Not_found
...
(** Pretty print an ['a iter], using the given pretty printer to print each elements. An optional
    separator string can be provided. *)
