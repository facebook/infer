...
type ('compare_elt, 'compare_mul) compare = ('compare_elt, 'compare_mul) Map.compare
[@@deriving compare, equal, sexp]
...
struct
  ...
  let add x i m = M.update x m ~f:(function Some j -> if_nz (Mul.add i j) | None -> if_nz i)
  ...
  let map m ~f =
    ...
    let m, m' =
      M.fold m (m, m') ~f:(fun ~key:x ~data:i (m, m') ->
          let x', i' = f x i in
          if x' == x then if Mul.equal i' i then (m, m') else (M.add ~key:x ~data:i' m, m')
