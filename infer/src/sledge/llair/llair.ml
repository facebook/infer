...
struct
  ...
  let bij =
    Iter.of_list I.all |> Iter.map ~f:(fun i -> (i, Sexp.to_string (I.sexp_of_t i))) |> B.of_iter
...
and callee = Direct of call_target | Indirect of icall_target | Intrinsic of Intrinsic.t
...
let compare_block x y = if x == y then 0 else Int.compare x.sort_index y.sort_index
...
with type jump := jump
 ...
 and type term := term = struct
  ...
  type nonrec call_target = call_target = {mutable func: func; mutable recursive: bool}
  [@@deriving compare, equal]

  type nonrec icall_target = icall_target = {ptr: Exp.t; mutable candidates: call_target iarray}
  [@@deriving compare, equal]
...
let sexp_cons (hd : Sexp.t) (tl : Sexp.t) =
  match tl with List xs -> Sexp.List (hd :: xs) | Atom _ -> Sexp.List [hd; tl]


let sexp_ctor label args = sexp_cons (Sexp.Atom label) args

let sexp_of_jump {dst; retreating} = [%sexp {dst: label = dst.lbl; retreating: bool}]

let sexp_of_callee = function
  | Direct {func; _} ->
      sexp_ctor "Direct" [%sexp (func.name : FuncName.t)]
  | Indirect {ptr; candidates= _} ->
      sexp_ctor "Indirect" [%sexp (ptr : Exp.t)]
  | Intrinsic intr ->
      sexp_ctor "Intrinsic" [%sexp (intr : Intrinsic.t)]
...
let pp_call_target fs {func; recursive} =
  Format.fprintf fs "%s%a" (if recursive then "↑" else "") FuncName.pp func.name


let pp_callee fs = function
  | Direct target ->
      pp_call_target fs target
  | Indirect {ptr; candidates} ->
      Format.fprintf fs "%a(candidates: %a)" Exp.pp ptr (IArray.pp " ; " pp_call_target) candidates
  | Intrinsic i ->
      Intrinsic.pp fs i
...
let pp_block fs {lbl; cmnd; term; parent= _; sort_index; goal_distance= _} =
  Format.fprintf fs "@[<v 2>%%%s: #%i@ @[<v>%a%t%a@]@]" lbl sort_index pp_cmnd cmnd
...
module Inst = struct
  ...
  let atomic_rmw ~reg ~ptr ~exp ~len ~loc = AtomicRMW {reg; ptr; exp; len; loc}
  ...
  let fold_exps inst s ~f =
    ...
    | Load {reg= _; ptr; len; loc= _} ->
        f len (f ptr s)
    | Store {ptr; exp; len; loc= _} ->
        f len (f exp (f ptr s))
    | AtomicRMW {reg= _; ptr; exp; len; loc= _} ->
        f len (f exp (f ptr s))
    | AtomicCmpXchg {reg= _; ptr; cmp; exp; len; len1; loc= _} ->
        f len1 (f len (f exp (f cmp (f ptr s))))
    | Alloc {reg= _; num; len= _; loc= _} ->
        f num s
    | Free {ptr; loc= _} ->
        f ptr s
    | Nondet {reg= _; msg= _; loc= _} ->
        s
    | Builtin {reg= _; name= _; args; loc= _} ->
        IArray.fold ~f args s
end
...
module Term = struct
  ...
  let goto ~dst ~loc = Switch {key= Exp.false_; tbl= IArray.empty; els= dst; loc} |> check invariant
  ...
  let switch ~key ~tbl ~els ~loc = Switch {key; tbl; els; loc} |> check invariant
  ...
  let icall ~callee ~typ ~actuals ~areturn ~return ~throw ~loc =
    ...
    let cal = {callee= Indirect target; typ; actuals; areturn; return; throw; loc} in
    let k = Call cal in
    ( k |> check invariant
    , fun ~candidates ->
        target.candidates <- IArray.map candidates ~f:(fun func -> {func; recursive= false}) )


  let intrinsic ~callee ~typ ~actuals ~areturn ~return ~throw ~loc =
    Call {callee= Intrinsic callee; typ; actuals; areturn; return; throw; loc} |> check invariant
...
module Block = struct
  ...
  let pp_ident fs b =
    if String.is_empty b.lbl then Format.fprintf fs "%a" FuncName.pp b.parent.name
...
module IP = struct
  ...
  let inst {block; index} =
    if index < IArray.length block.cmnd then Some (IArray.get block.cmnd index) else None
  ...
  let loc ip = match inst ip with Some i -> Inst.loc i | None -> Term.loc (block ip).term

  let is_schedule_point ip =
    if !cct_schedule_points then
      match inst ip with Some (Builtin {name= `cct_point; _}) -> true | _ -> false
    else
      match inst ip with
      ...
      | Some (Builtin {name; _}) -> (
        match name with
        | `calloc | `malloc | `mallocx | `nallocx | `cct_point ->
            false
      ...
      | None -> (
        ...
        | Call {callee= Direct {func; _}; _} -> (
          match FuncName.name func.name with "sledge_thread_join" -> true | _ -> false )
        | _ ->
            false )
...
module Block_label = struct
  module T = struct
    ...
    let compare x y = [%compare: string * FuncName.t] (x.lbl, x.parent.name) (y.lbl, y.parent.name)

    let equal x y = [%equal: string * FuncName.t] (x.lbl, x.parent.name) (y.lbl, y.parent.name)
...
module Func = struct
  ...
  let pp_call fs call =
    ...
    let pp_arg fs (actual, formal) = Format.fprintf fs "@[%a / %a@]" Exp.pp actual Reg.pp formal in
    let pp_args fs (actuals, formals) =
      IArray.pp ",@ " pp_arg fs (IArray.combine actuals formals)
    in
    Format.fprintf fs "%a%a(@[%a@])" (Option.pp "%a := " pp_arg)
  ...
  let find name functions = FuncName.Map.find (FuncName.counterfeit name) functions
  ...
  let mk ~name ~formals ~freturn ~fthrow ~entry ~cfg ~loc =
    ...
    let rec resolve_parent_and_jumps ancestors src =
      ...
      | Switch {tbl; els; _} ->
          ...
          let tgt = jump els in
          if IArray.is_empty tbl && IArray.is_empty src.cmnd then Some tgt else None
      | Iswitch {tbl; _} ->
...
let set_derived_metadata functions =
  let compute_roots functions =
    ...
    FuncName.Map.iter functions ~f:(fun func -> FuncQ.enqueue_back_exn roots func func) ;
  ...
  let topsort roots =
    ...
    let rec visit ancestors src =
      if BlockQ.mem tips_to_roots src then ()
        ...
        let ancestors = Block_label.Set.add src ancestors in
        let jump jmp = if jmp.retreating then () else visit ancestors jmp.dst in
        ( match src.term with
        | Switch {tbl; els; _} ->
            IArray.iter tbl ~f:(fun (_, jmp) -> jump jmp) ;
            jump els
        | Iswitch {tbl; _} ->
            IArray.iter tbl ~f:jump
        | Call {callee; return; throw; _} ->
            ( match callee with
            | Direct tgt ->
                if Block_label.Set.mem tgt.func.entry ancestors then tgt.recursive <- true
                else visit ancestors tgt.func.entry
            | Indirect {candidates; _} ->
                IArray.iter candidates ~f:(fun c ->
                    if Block_label.Set.mem c.func.entry ancestors then c.recursive <- true
    ...
    in
    FuncQ.iter roots ~f:(fun root -> visit Block_label.Set.empty root.entry) ;
    tips_to_roots
  in
