  ...
  with Not_found ->
    ...
    | Some _
     -> L.(debug Capture Verbose)
          "Warning: Wrong decl found for  pointer %s " (Clang_ast_j.string_of_pointer decl_ptr) ;
        Typ.Tvoid
    | None
     -> L.(debug Capture Verbose)
          "Warning: Decl pointer %s not found." (Clang_ast_j.string_of_pointer decl_ptr) ;
        Typ.Tvoid
  ...
  with Not_found ->
    ...
    | Some c_type
     -> let type_desc = type_desc_of_c_type translate_decl tenv c_type in
        CAst_utils.update_sil_types_map type_ptr type_desc ; type_desc
    | _
     -> Typ.Tvoid
