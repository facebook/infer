...
let get_method_procname cn ms method_kind =
  ...
  let proc_name_java =
    Typ.Procname.java class_name return_type_name method_name args_type_name method_kind
  in
    ...
    if Typ.equal classpath_ft field_type then fields
      ...
      let () =
        L.(debug Capture Quiet)
          ...
          (Typ.Fieldname.to_string field_name) (Typ.pp_full Pp.text) classpath_ft
          (Typ.pp_full Pp.text) field_type
      in
      fields
  with Not_found ->
...
let add_model_fields program classpath_fields cn =
  ...
  try
    ...
    | Javalib.JClass _ as jclass
     -> Javalib.cf_fold (collect_models_class_fields classpath_field_map cn) jclass
          classpath_fields
    | _
     -> classpath_fields
...
let is_autogenerated_assert_field field_name =
  String.equal (Typ.Fieldname.java_get_field field_name) "$assertionsDisabled"
