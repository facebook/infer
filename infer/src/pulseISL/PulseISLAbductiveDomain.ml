...
type t =
  ...
  ; status: PostStatus.t
  ; abducible_vars : VarSet.t
  ; local_vars : VarSet.t
  ; max_logvar: AbstractValue.t}

let pp f {post; pre; path_condition; skipped_calls; max_logvar; abducible_vars; local_vars;  status} =
  F.fprintf f "@[<v>%a\n%a@;%a@;PRE=[%a]@;skipped_calls=%a@;abducible_vars=%a@;local_vars=%a@;max_logvar=%a@]" PathCondition.pp path_condition
      PostStatus.pp status PostDomain.pp post PreDomain.pp pre SkippedCalls.pp skipped_calls
      VarSet.pp abducible_vars VarSet.pp local_vars
      AbstractValue.pp max_logvar

let set_status status astate =
  {astate with status}

let add_abducible_var var astate =
  {astate with abducible_vars = VarSet.add var astate.abducible_vars}

let remove_abducible_vars vars astate =
  {astate with abducible_vars = List.fold vars ~init:astate.abducible_vars ~f:(fun acc var -> VarSet.remove var acc)}

let set_max_logvar astate =
  let  max_logvar = AbstractValue.get_max () in
  {astate with max_logvar}

let set_path_condition path_condition astate =
...
module Stack = struct
  ...
  let eval location origin var astate =
    ...
    | Some addr_hist ->
        (astate, addr_hist)
    | None -> (*for some ghost e.g., ret variable, for example*)
        let addr = AbstractValue.mk_fresh () in
        let addr_hist = (addr, origin) in
        let post_stack = BaseStack.add var addr_hist (astate.post :> base_domain).stack in
        let post_heap = BaseMemory.register_address addr (astate.post :> base_domain).heap in
        let post_attrs = let access_trace = Trace.Immediate {location; history=[]} in
        ...
        ( { post= PostDomain.update astate.post ~stack:post_stack ~heap:post_heap ~attrs:post_attrs
          ...
          ; abducible_vars = astate.abducible_vars
          ; local_vars = astate.local_vars
          ; max_logvar = AbstractValue.get_max ()}
        , addr_hist )
...
module AddressAttributes = struct
  ...
  let abduce_and_add value attrs astate =
    Attributes.fold attrs ~init:astate ~f:(fun astate attr ->
        let astate =
          if Attribute.is_suitable_for_pre attr then
              abduce_attribute value attr astate
          else
              astate
        in
   ...
   let abduce_attrs value attrs astate =
    Attributes.fold attrs ~init:astate ~f:(fun astate attr ->
        let astate =
          if Attribute.is_suitable_for_pre attr then
              abduce_attribute value attr astate else
              astate
        in
        astate )

   let add_attrs value attrs astate =
     let astate = match Attributes.get_invalid attrs with | Some _ -> remove_must_be_valid_attr value astate | _ -> astate in
     Attributes.fold attrs ~init:astate ~f:(fun astate attr ->
             add_one value attr
                 astate)
  ...
  let is_abducible address astate=
    if (BaseMemory.exist_edge_dest address (astate.pre :> base_domain).heap) then
        true
    else
      let abduced_vars = astate.abducible_vars in
     (BaseDomain.is_aliasing_abducible abduced_vars address (astate.post :> BaseDomain.t))
  ...
  let check_valid_and_abduce procname access_trace addr ?null_noop:(null_noop=false) is_neq_null is_pc_eq_null astate =
    ...
    | None -> begin
      ...
      | None -> (
          ...
          if not (is_abducible addr astate) then
              ...
              let null_astates =
                if not is_neq_null then
                    ...
                    let null_astate = { astate with status = if null_noop then astate.status else PostStatus.Er; path_condition = PathCondition.and_eq_int addr IntLit.zero astate.path_condition}
              ...
              if is_pc_eq_null then Ok null_astates else
                  let valid_astate =
                    ...
                    add_one addr abdalloc {astate with path_condition = PathCondition.and_positive addr astate.path_condition}
                    (* |> add_one addr valid_attr *) (* must be valid attribute just for pre *)
                    |> abduce_attribute addr valid_attr in
              (* invalid *)
              let invalid_astate =
                (*C or Cpp?*)
                let invalid_attr = Attribute.Invalid (CFree, access_trace) in
                { astate with status = PostStatus.Er;path_condition = PathCondition.and_positive addr astate.path_condition}
                (* abduce invalid heap into both pre and post *)
                |> abduce_attribute addr invalid_attr
                |> add_one addr invalid_attr in
              Ok ([valid_astate;invalid_astate]@null_astates))
      | Some _ -> (
          (* dont need abduce *)
          Ok [astate])
        end
...
let mk_initial proc_desc =
  ...
  let formal_vars, initial_stack =
    List.fold formals ~init:(VarSet.empty, (PreDomain.empty :> BaseDomain.t).stack)
      ~f:(fun (vars, stack) (formal, _, addr_loc) -> VarSet.add formal vars,BaseStack.add formal addr_loc stack)
  in
  let pre =
    let initial_heap =
      List.fold formals ~init:(PreDomain.empty :> base_domain).heap ~f:(fun heap (_, typ, (addr, _)) -> let h = BaseMemory.register_address addr heap in
                                                                                                        match typ.Typ.desc with                                                                         | Typ.Tptr _ ->
              let addr_dst = AbstractValue.mk_fresh () in
              BaseMemory.add_edge addr Dereference (addr_dst, []) h
              |> BaseMemory.register_address addr_dst
          | _ -> h)
    in
    let initial_attrs =
  ...
  in
  let post = PostDomain.update ~stack:initial_stack ~heap:((pre :> base_domain).heap) ~attrs:((pre :> base_domain).attrs) PostDomain.empty in
  let local_vars, bytecode_tmp =
      (Procdesc.get_locals proc_desc)
    |> List.fold ~init:(Var.Set.empty, []) ~f:(fun (vars, bc_vars) {ProcAttributes.name} ->
               let pvar = Pvar.mk name proc_name in
               if Pvar.is_bytecode_tmp_sv pvar then
                   (vars, bc_vars@[Var.of_pvar pvar])
               else
               (Var.Set.add (Var.of_pvar pvar) vars, bc_vars))
  in
  {pre; post; skipped_calls= SkippedCalls.empty; path_condition= PathCondition.true_;
   status = Ok; max_logvar = AbstractValue.get_max ()
   ; abducible_vars = List.fold bytecode_tmp ~init:formal_vars ~f:(fun acc v -> VarSet.add v acc)
