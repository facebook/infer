...
module Memory = PulseISLBaseMemory
module Stack = PulseISLBaseStack
module AddressAttributes = PulseISLBaseAddressAttributes
...
let is_empty spec= Memory.is_empty spec.heap && Stack.is_empty spec.stack &&
                       AddressAttributes.is_empty spec.attrs
...
let reachable_address_vars astate =
  GraphVisit.fold astate
    ~var_filter:(fun _ -> true)
    ~init:[] ~finish:Fn.id
    ~f:(fun acc _ var _ -> Continue (acc@[var]))
        
let reachable_addresses astate = reachable_addresses_from (fun _ -> true) astate

let reachable_backing_vars (backing_addresses,abduced_vars) astate =
  GraphVisit.fold astate
    ~var_filter:(fun v -> not (Var.Set.mem v abduced_vars))
    ~init:abduced_vars ~finish:Fn.id
    ~f:(fun acc addr var _ ->
        let n_acc = if AbstractValue.Set.mem addr backing_addresses then
                        (Var.Set.add var acc) else acc in
        Continue n_acc)
  |> snd
...
let is_aliasing_abducible abduced_vars address astate =
  let abducible_addrs = reachable_addresses_from (fun v -> Var.Set.mem v abduced_vars) astate in
  if AbstractValue.Set.mem address abducible_addrs then
    true
  else
    let nonlocal_vars = reachable_backing_vars (abducible_addrs, abduced_vars) astate in
    is_reachable address nonlocal_vars astate
