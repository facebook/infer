...
let rec pp fs typ =
  ...
  | Float {bits; enc} ->
      let enc_str =
        match enc with `Brain -> "brain" | `IEEE -> "" | `Extended -> "extend" | `Pair -> "pair"
      in
...
let pp_defn fs = function
  ...
  | Opaque {name} ->
      Format.fprintf fs "@[<2>%%%s =@ opaque@]" name
  | typ ->
      pp fs typ
...
let array ~elt ~len ~bits ~byts = Array {elt; len; bits; byts} |> check invariant
...
(** [ptr] is semantically equivalent to [siz], but has a distinct representation because the element
    type is important for [Global]s *)
...
let bit_size_of = function
  ...
  | Integer {bits; _} | Float {bits; _} | Array {bits; _} | Tuple {bits; _} | Struct {bits; _} ->
      bits
  | Pointer _ ->
      64
...
let size_of = function
  ...
  | Integer {byts; _} | Float {byts; _} | Array {byts; _} | Tuple {byts; _} | Struct {byts; _} ->
      byts
  | Pointer _ ->
      8
...
let offset_length_of_elt typ idx =
  ...
  | Tuple {elts; byts} | Struct {elts; byts} ->
      ...
      let oJ = if idx = IArray.length elts - 1 then byts else fst (IArray.get elts (idx + 1)) in
      (oI, oJ - oI)
  | _ ->
      fail "offset_length_of_elt: %a" pp typ ()
...
let castable t0 t1 = (is_sized t0 && is_sized t1 && bit_size_of t0 = bit_size_of t1) || equal t0 t1
...
let compatible_fnptr t t' =
  ...
  ||
  match (t, t') with
  | Pointer {elt= Function {return; args}}, Pointer {elt= Function {return= return'; args= args'}}
    ->
