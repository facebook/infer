...
and call_target =
  { mutable func: func
  ; mutable recursive: bool  (** Holds unless this call target is definitely not recursive *) }
...
and callee =
  ...
  | Intrinsic of Intrinsic.t  (** Intrinsic implemented in analyzer rather than source code *)
...
and block = private
  ...
  ; term: term
  ; mutable parent: func
  ; mutable sort_index: int  (** Position in a topological order, ignoring [retreating] edges. *)
  ; mutable goal_distance: int
        (** An upper bound on the distance from this block to the end of the current goal trace,
            measured in blocks. *) }
