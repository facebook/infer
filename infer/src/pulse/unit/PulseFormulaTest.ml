    ...
    let%expect_test _ =
      ...
      [%expect
        ...
        phi: var_eqs: v8=v9=v10
             ...
             && term_conds: {[3·y -v6] = 0}∧{[12·v8 -v9] = 0}|}]
      ...
      [%expect
        {|
          ...
          phi: var_eqs: x=v6=v7=v8=v9=v10
               ...
               && term_eqs: 0=x
               && intervals: x=0
               && term_conds: {x = 0}∧{v6 = 0}∧{v7 = 0}∧{v8 = 0}∧{v9 = 0} |}]
  ...
  ( module struct
    let%expect_test _ =
      ...
      [%expect
        {|
        conditions: {x = 42}
        phi: linear_eqs: x = 42 && term_eqs: 42=x && intervals: x=42 && term_conds2: {x=42} |}]


    let%expect_test _ =
      test (x =. i 42 || x <>. i 42) ;
      [%expect {|
        conditions: (empty) phi: term_conds2: {x=42} |}]
    ...
    let%expect_test _ =
      ...
      [%expect
        {|
        conditions: (empty) phi: linear_eqs: x = y +1 && term_eqs: [y +1]=x && term_conds2: {x=v6} |}]


    let%expect_test _ =
      test (z = y + i 1 && (x =. z || x <>. z)) ;
      [%expect {|
        conditions: (empty) phi: var_eqs: x=v6 && term_conds2: {x=z} |}]
    ...
    let%expect_test _ =
      ...
      [%expect {|
        conditions: (empty) phi: term_conds2: {x=42}∧{y=w} |}]


    let%expect_test _ =
      test (x =. y * z || i 0 = i 0) ;
      [%expect {|
        conditions: (empty) phi: term_conds2: {x=v6} |}]
    ...
    let%expect_test _ =
      ...
      [%expect
        {|
        conditions: {[x -w] = 0} phi: var_eqs: x=w=v6 && term_eqs: (y×z)=x && term_conds2: {x=w} |}]
    ...
    let%expect_test _ =
      ...
      [%expect
        {|
        conditions: {x = "toto"} phi: const_eqs: x="toto" && term_eqs: "toto"=x && term_conds2: {x="toto"} |}]


    let%expect_test _ =
      test (x =. s "toto" || x =. s "titi") ;
      [%expect {|
        conditions: (empty) phi: term_conds2: {x="toto"} |}]
  end )
