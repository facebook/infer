...
module Term = struct
  ...
  (* We use this to track constraints that should not be optimized for infinite loop detection *)
  module Set = struct
...
module Formula = struct
  ...
  module Unsafe : sig
    ...
    type t = private
      ...
      ; raw_atom_conds: Atom.Set.t  (** Termination conditions for pulse-inf *)
    ...
    let is_empty
      ...
      && Var.Map.is_empty intervals && Atom.Set.is_empty atoms && Atom.Set.is_empty raw_atom_conds
    ...
    let pp_with_pp_var pp_var fmt
      ...
      (pp_if
         (not (Atom.Set.is_empty raw_atom_conds))
         "raw_atom_conds" (Atom.Set.pp_with_pp_var pp_var) )
        fmt raw_atom_conds ;
      (pp_if
         (not (Term.Set.is_empty raw_term_conds))
         "raw_term_conds" (Term.Set.pp_with_pp_var pp_var) )
        fmt raw_term_conds ;
    ...
    let add_termination_atom atom phi =
      let atoms_occurrences =
        Atom.fold_variables atom ~init:phi.atoms_occurrences ~f:(fun occurrences v' ->
            AtomMapOccurrences.add v' ~occurs_in:atom occurrences )
      in
      {phi with raw_atom_conds= Atom.Set.add atom phi.raw_atom_conds; atoms_occurrences}
    ...
    let get_terminal_conds t = t.raw_atom_conds
      ...
      let term_eqs, term_eqs_occurrences =
        ...
        | Some term_eqs_v ->
            TermDomainOrRange.Set.fold
              ...
              (phi.term_eqs, phi.term_eqs_occurrences)
      in
      let raw_atom_conds = phi.raw_atom_conds in
  ...
  module Normalizer : sig
    ...
    val and_atom : Atom.t -> t * new_eqs -> bool -> (t * new_eqs) SatUnsat.t
    ...
    and and_normalized_atoms (phi, new_eqs) atoms ~orig_atom ~add_term =
      (* Possibly useful: test if variables in orig_atom are present in term_conds:
         if so, add orig_atom in term_conds even if add_term is false.
         Note: this may not be necessary thanks to Pulse fresh name creation which
         is sufficient to witness the existence of a state update without tracking
         intermediate constraints *)
      let upd_phi = if add_term then and_termcond_atoms phi orig_atom else phi in
...
module Intervals = struct
  ...
  let and_binop ~negated binop op1 op2 ?(ifk = false) (formula, new_eqs) =
    ...
    | Satisfiable (i1_better_opt, i2_better_opt) ->
        ...
        in
        let nformula =
          if phys_equal ifk false || binop_unknown binop then formula
          else
            let atom_to_binop (binop : Binop.t) =
              ...
              | _ ->
                  L.die InternalError "atom_to_binop: Wrong arg: %a should never happen! " Binop.pp
                    binop
            in
            let inv, op = atom_to_binop binop in
            let invcond = inv in
            (* Handle cases like: while (x == x) for non-termination for loops *)
            let opcond =
...
let and_mk_atom binop op1 op2 formula =
  ...
  let+ formula, new_eqs' = and_atom atom formula false in
...
let and_equal_unop v (op : Unop.t) x formula =
  ...
  and_atom (Equal (Var v, Term.of_unop op (Term.of_operand x))) formula false


let and_equal_binop v (bop : Binop.t) x y formula =
  let* formula = Intervals.binop v bop x y formula in
  and_atom (Equal (Var v, Term.of_binop bop (Term.of_operand x) (Term.of_operand y))) formula false
...
let prune_atoms ~depth atoms formula_new_eqs ifk =
...
let prune_binop ?(depth = 0) ~negated (bop : Binop.t) ?(ifk = false) x y formula =
  ...
  (* NOTE: [Intervals.and_binop] may tip off the rest of the formula about the new equality so it's
     ...
     4]. *)
  prune_atoms ~depth atoms (newformula, RevList.empty) ifk
  >>= Intervals.and_binop ~negated bop x y ~ifk
  ...
  let and_atoms atoms_foreign acc_phi_new_eqs =
    IContainer.fold_of_pervasives_set_fold Atom.Set.fold atoms_foreign ~init:acc_phi_new_eqs
      ~f:(fun (acc_f, phi_new_eqs) atom_foreign ->
        ...
        let phi_new_eqs = Formula.Normalizer.and_atom atom phi_new_eqs false |> sat_value_exn in
        (acc_f, phi_new_eqs) )
  in
...
module QuantifierElimination : sig
  ...
  let subst_var_phi subst
    ...
    Formula.unsafe_mk ~var_eqs:(VarUF.apply_subst subst var_eqs)
      ...
      ~raw_atom_conds:Atom.Set.empty ~raw_term_conds:Term.Set.empty
