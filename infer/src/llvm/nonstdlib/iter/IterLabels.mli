...
(** An iterator of values of type ['a]. If you give it a function ['a -> unit] it will be applied to
    every element of the iterator successively. *)
...
(** {b NOTE} Type [('a, 'b) t2 = ('a -> 'b -> unit) -> unit] has been removed and subsumed by
    [('a * 'b) t]
...
val repeat : 'a -> 'a t
(** Infinite iterator of the same element. You may want to look at {!take} and the likes if you
    iterate on it. *)
...
(** Iterator that calls the given function to produce elements. The iterator may be transient
    (depending on the function), and definitely is infinite. You may want to use {!take} and
    {!persistent}. *)

val cycle : 'a t -> 'a t
(** Cycle forever through the given iterator. Assume the given iterator can be traversed any amount
    of times (not transient). This yields an infinite iterator, you should use something like
    {!take} not to loop forever. *)

(** {2 Consumption} *)

val iter : 'a t -> f:('a -> unit) -> unit
(** Consume the iterator, passing all its arguments to the function. Basically [iter ~f seq] is just
    [seq f]. *)
...
(** Consume the iterator, passing all its arguments to the function. [for_each seq f] is the same as
    [iter ~f seq], i.e., [iter] with arguments reversed.

    @since 1.4 *)

val for_eachi : 'a t -> (int -> 'a -> unit) -> unit
(** Iterate on elements and their index in the iterator. [for_eachi seq f] is the same as
    [iteri ~f seq], i.e., [iteri] with arguments reversed.
...
(** [fold_map seq s ~f] is like {!map}, but it carries some state as in {!fold}. The final state is
    returned. *)

val folding_map : 'a t -> 's -> f:('a -> 's -> 'b * 's) -> 'b t
(** [folding_map seq s ~f] is like {!map}, but it carries some state as in {!fold}. The state is not
    returned, it is just used to thread some information to the map function.

    @since 0.9 *)

val fold_filter_map : 'a t -> 's -> f:('a -> 's -> 'b option * 's) -> 'b t
(** [fold_filter_map seq s ~f] is a {!fold_map}-like function, but the function can choose to skip
    an element by retuning [None].
...
(** [find seq ~f] finds the first element of [seq] that satisfies [f], or returns [None] if no
    element satisfies [f]
...
(** Append two iterators. Iterating on the result is like iterating on the first, then on the
    second. *)

val append_l : 'a t list -> 'a t
(** Append iterators. Iterating on the result is like iterating on the each iterator of the list in
    order.
...
(** Monadic bind. Intuitively, it applies the function to every element of the initial sequence, and
    calls {!concat}. *)
...
(** [seq_list l] returns all the ways to pick one element in each sub-iterator in [l]. Assumes the
    sub-iterators can be iterated on several times.
...
(** Iterate on the iterator, storing elements in an efficient internal structure.. The resulting
    iterator can be iterated on as many times as needed. {b Note}: calling persistent on an already
    persistent iterator will still make a new copy of the iterator! *)

val persistent_lazy : 'a t -> 'a t
(** Lazy version of {!persistent}. When calling [persistent_lazy s], a new iterator [s'] is
    immediately returned (without actually consuming [s]) in constant time; the first time [s'] is
    iterated on, it also consumes [s] and caches its content into a inner data structure that will
    back [s'] for future iterations.

    {b warning}: on the first traversal of [s'], if the traversal is interrupted prematurely
    ({!take}, etc.) then [s'] will not be memorized, and the next call to [s'] will traverse [s]
    again. *)

(** {2 Misc} *)

val sort : 'a t -> cmp:('a -> 'a -> int) -> 'a t
(** Sort the iterator. Eager, O(n) ram and O(n ln(n)) time. It iterates on elements of the argument
    iterator immediately, before it sorts them. *)
...
(** Group equal elements, disregarding their order of appearance. The result iterator is traversable
    as many times as required. precondition: for any [x] and [y], if [eq x y] then [hash x=hash y]
    must hold.

    @since 0.6 *)

val count : 'a t -> hash:('a -> int) -> eq:('a -> 'a -> bool) -> ('a * int) t
...
(** Cartesian product of the iterators. When calling [product a b], the caller {b MUST} ensure that
    [b] can be traversed as many times as required (several times), possibly by calling
    {!persistent} on it beforehand. *)

val diagonal_l : 'a list -> ('a * 'a) t
(** All pairs of distinct positions of the list. [diagonal l] will return the iterator of all
    [List.nth i l, List.nth j l] if [i < j].

    @since 0.9 *)

val diagonal : 'a t -> ('a * 'a) t
(** All pairs of distinct positions of the iterator. Iterates only once on the iterator, which must
    be finite.

    @since 0.9 *)

val join : 'a t -> 'b t -> f:('a -> 'b -> 'c option) -> 'c t
(** [join ~f:join_row a b] combines every element of [a] with every element of [b] using [join_row].
    If [join_row] returns [None], then the two elements do not combine. Assume that [b] allows for
    multiple iterations. *)
...
val join_by :
  ...
  -> f:('key -> 'a -> 'b -> 'c option)
  -> 'c t
(** [join key1 key2 ~f:merge] is a binary operation that takes two iterators [a] and [b], projects
    their elements resp. with [key1] and [key2], and combine values [(x,y)] from [(a,b)] with the
    same [key] using [merge]. If [merge] returns [None], the combination of values is discarded.
...
val join_all_by :
  ...
  -> 'c t
(** [join_all_by key1 key2 ~f:merge] is a binary operation that takes two iterators [a] and [b],
    projects their elements resp. with [key1] and [key2], and, for each key [k] occurring in at
    least one of them:

    - compute the list [l1] of elements of [a] that map to [k]
    - compute the list [l2] of elements of [b] that map to [k]
    - call [merge k l1 l2]. If [merge] returns [None], the combination of values is discarded,
      otherwise it returns [Some c] and [c] is inserted in the result.

    @since 0.10 *)

val group_join_by : 'a t -> 'b t -> eq:'a equal -> hash:'a hash -> f:('b -> 'a) -> ('a * 'b list) t
(** [group_join_by ~f:key] associates to every element [x] of the first iterator, all the elements
    [y] of the second iterator such that [eq x (key y)]. Elements of the first iterators without
    corresponding values in the second one are mapped to [[]] precondition: for any [x] and [y], if
    [eq x y] then [hash x=hash y] must hold.
...
(** [subset a b] returns [true] if all elements of [a] belong to [b]. Eager. precondition: for any
...
(** [unfoldr ~f b] will apply [f] to [b]. If it yields [Some (x,b')] then [x] is returned and
    unfoldr recurses with [b']. *)
...
(** Max element of the iterator, using the given comparison function.
    ...
    @return None if the iterator is empty, Some [m] where [m] is the maximal element otherwise *)
...
(** Take elements while they satisfy the predicate, then stops iterating. Will work on an infinite
    iterator [s] if the predicate is false for at least one element of [s]. *)

val fold_while : 'b t -> 'a -> f:('b -> 'a -> [`Stop | `Continue] * 'a) -> 'a
...
(** Folds over elements of the iterator, stopping early if the accumulator returns [None]. *)

val fold_result : 'a t -> 's -> f:('a -> 's -> ('s, 'e) result) -> ('s, 'e) result
(** Folds over elements of the iterator, stopping early if the accumulator returns [Error]. *)

val fold_until :
  'a t -> 's -> f:('a -> 's -> [`Continue of 's | `Stop of 'b]) -> finish:('s -> 'b) -> 'b
(** Folds over elements of the iterator, stopping early if the accumulator returns [`Stop b], and
    using [finish] to transform the final state if the end of the iterator is reached. *)
...
val rev : 'a t -> 'a t
(** Reverse the iterator. O(n) memory and time, needs the iterator to be finite. The result is
    persistent and does not depend on the input being repeatable. *)
...
val map2_2 : ('a * 'b) t -> f:('a -> 'b -> 'c) -> g:('a -> 'b -> 'd) -> ('c * 'd) t
(** [map2_2 ~f ~g seq2] maps each [x, y] of seq2 into [f x y, g x y] *)

(** {2 Data structures converters} *)

val to_list : 'a t -> 'a list
(** Convert the iterator into a list. Preserves order of elements. This function is tail-recursive,
    but consumes 2*n memory. If order doesn't matter to you, consider {!to_rev_list}. *)
...
(** Convert to a {!Seq}. Linear in memory and time (a copy is made in memory). This does not work on
    infinite iterators.
...
(** Add elements of the iterator to the hashtable, with Hashtbl.replace (erases conflicting
    bindings) *)
...
(** Iterates on characters of the input (can block when one iterates over the iterator). If you need
    to iterate several times on this iterator, use {!persistent}.
...
(** [int_range_by ~step ~start:i ~stop:j] is the range starting at [i], including [j], where the
    difference between successive elements is [step]. use a negative [step] for a decreasing
    iterator.
...
val of_set : (module Iterable with type elt = 'a and type t = 'b) -> 'b -> 'a t
...
val to_set : (module Addable with type elt = 'a and type t = 'b) -> 'a t -> 'b
...
(** [shuffle_buffer n seq] returns an iterator of element of [seq] in random order. The shuffling is
    not uniform. Uses O(n) memory.
...
(** [sample n seq] returns k samples of [seq], with uniform probability. It will consume the
    iterator and use O(n) memory.
...
module Infix : sig
  val ( -- ) : int -> int -> int t
  (** [a -- b] is the range of integers from [a] to [b], both included, in increasing order. It will
      therefore be empty if [a > b]. *)

  val ( --^ ) : int -> int -> int t
  (** [a --^ b] is the range of integers from [b] to [a], both included, in decreasing order (starts
      from [a]). It will therefore be empty if [a < b]. *)
...
(** Pretty print an iterator of ['a], using the given pretty printer to print each elements. An
    optional separator string can be provided. *)
...
(** {2 Basic IO}
    ...
    Very basic interface to manipulate files as iterator of chunks/lines. The iterators take care of
    opening and closing files properly; every time one iterates over an iterator, the file is
    opened/closed again.
...
module IO : sig
  ...
  (** [lines_of filename] reads all lines of the given file. It raises the same exception as would
      opening the file and read from it, except from [End_of_file] (which is caught). The file is
      {b always} properly closed. Every time the iterator is iterated on, the file is opened again,
      so different iterations might return different results

      @param mode default [0o644]
      @param flags default: [[Open_rdonly]] *)

  val chunks_of : ?mode:int -> ?flags:open_flag list -> ?size:int -> string -> string t
  (** Read chunks of the given [size] from the file. The last chunk might be smaller. Behaves like
      {!lines_of} regarding errors and options. Every time the iterator is iterated on, the file is
      opened again, so different iterations might return different results *)

  val write_to : ?mode:int -> ?flags:open_flag list -> string -> string t -> unit
  (** [write_to filename seq] writes all strings from [seq] into the given file. It takes care of
      opening and closing the file.

      @param mode default [0o644]
      @param flags used by [open_out_gen]. Default: [[Open_creat;Open_wronly]]. *)

  val write_bytes_to : ?mode:int -> ?flags:open_flag list -> string -> Bytes.t t -> unit
  (** @since 0.5.4 *)

  val write_lines : ?mode:int -> ?flags:open_flag list -> string -> string t -> unit
  (** Same as {!write_to}, but intercales ['\n'] between each string *)

  val write_bytes_lines : ?mode:int -> ?flags:open_flag list -> string -> Bytes.t t -> unit
  (** @since 0.5.4 *)
end
