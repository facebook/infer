...
val pp : ?pre:(unit, unit) fmt -> ?suf:(unit, unit) fmt -> (unit, unit) fmt -> 'a pp -> 'a list pp
(** Pretty-print a list. *)
...
(** Returns the input list without the first element [eq]ual to the argument, or raise [Not_found]
...
(** [partition xs ~f] returns a pair of lists [(ts, fs)], where [ts] is the list of all the elements
    of [xs] that satisfy the predicate [f], and [fs] is the list of all the elements of [xs] that do
    not satisfy [f]. The order of the elements in the input list is preserved. *)

val partition_map : 'a t -> f:('a -> ('l, 'r) Either.t) -> 'l t * 'r t
(** [partition_map xs ~f] returns a pair of lists [(ls, rs)] such that, for each element [x] of the
    input list [xs]:

    - if [f x] is [Left l], then [l] is in [ls], and
    - if [f x] is [Right r], then [r] is in [rs].

    The output elements are included in [ls] and [rs] in the same relative order as the
    corresponding input elements in [xs]. *)

val rev_partition_map : 'a t -> f:('a -> ('l, 'r) Either.t) -> 'l t * 'r t
(** [rev_partition_map xs ~f] is [(rev ls, rev rs)] where [partition_map xs ~f = (ls, rs)]. This
    avoids the [rev] that [partition_map] performs. *)

val partition_map_endo : 'a t -> f:('a -> ('a, 'r) Either.t) -> 'a t * 'r t
(** [partition_map_endo xs ~f] is equivalent to [partition_map xs ~f] but specialized to the case
    where the left and input elements have the same type. Preserves [==] if [f x] is [Left x] for
    all [x] in [xs]. *)

val fold_partition_map : 'a t -> 's -> f:('a -> 's -> ('l, 'r) Either.t * 's) -> 'l t * 'r t * 's
(** [fold_partition_map] is like [partition_map] but threads an accumulator through the calls to
    [f]. *)

val fold_partition_map_endo :
  'a t -> 's -> f:('a -> 's -> ('a, 'r) Either.t * 's) -> 'a t * 'r t * 's
(** [fold_partition_map_endo xs s ~f] is equivalent to [fold_partition_map xs s ~f] but specialized
    to the case where the left and input elements have the same type. Preserves [==] if [f x] is
...
val group_join_by :
  eq:('a -> 'a -> bool) -> hash:('a -> int) -> ('b -> 'a) -> 'a t -> 'b t -> ('a * 'b list) t
