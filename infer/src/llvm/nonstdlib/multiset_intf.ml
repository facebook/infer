...
module type S = sig
  ...
  val pp :
    ?pre:(unit, unit) fmt -> ?suf:(unit, unit) fmt -> (unit, unit) fmt -> (elt * mul) pp -> t pp
  ...
  (** Flat map over the elements in ascending order. Preserves physical equality if [f e m] is a
      singleton [(e', m')] with [e == e'] and [Mul.equal m m'] for all elements. *)
