        ...
        (fun l v1_opt v2_opt ->
          ...
          | Some v1, None | None, Some v1 ->
              (* Since abstract state doesn't contain complete information, use Val.top
                      if a location is missing in one state being joined. *)
              let v2 = MVal.on_demand ~default:Val.top oenv l in
              Some (MVal.join v1 v2)
          | None, None ->
              None )
        astate1 astate2
    ...
    if phys_equal prev next then prev
      ...
      merge
        (fun l v1_opt v2_opt ->
          ...
          | Some v1, None ->
              (* Since abstract state doesn't contain complete information, use Val.top
                      if a location is missing in one state being widened. *)
              let v2 = MVal.on_demand ~default:Val.top oenv l in
              Some (MVal.widen ~prev:v1 ~next:v2 ~num_iters)
          | None, Some v2 ->
              (* Since abstract state doesn't contain complete information, use Val.top
                 if a location is missing in one state being widened. *)
              let v1 = MVal.on_demand ~default:Val.top oenv l in
              Some (MVal.widen ~prev:v1 ~next:v2 ~num_iters)
          | None, None ->
              None )
        prev next
...
module MemReach = struct
  ...
  let find_heap : ?typ:Typ.t -> Loc.t -> _ t0 -> Val.t =
   fun ?typ l m -> find_heap_default ~default:Val.top ?typ l m
    ...
    let transform_mem1 l m =
      let add, find =
        if is_stack_loc l m then (replace_stack, find_stack)
          ...
          (add_heap ~represents_multiple_values:false, find_heap_default ~default:Val.top ?typ:None)
      in
