 ...
 fun integer_type_widths -> function
  ...
  | Const.Cfun _ ->
      (* Const.Cfun represents the address of a function call.
         ...
         TODO: set func_ptrs field to be a singleton set containing the function *)
        ...
        (fun loc acc ->
          Val.join
            (let v = Mem.find loc mem in
             if Val.is_top v then
               (* There is no or unknown value for the dereference of a pointer in the abstract
...
module Prune = struct
  ...
  let prune_stack_var pruned_id prune ({mem} as astate) =
    ...
    (pruned_val, {astate with mem; complete= astate.complete && Val.is_precise pruned_val})
