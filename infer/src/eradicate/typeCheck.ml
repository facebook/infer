...
let typecheck_instr tenv ext calls_this checks (node: Procdesc.Node.t) idenv get_proc_desc
  ...
  let constructor_check_calls_this calls_this pn =
    ...
    | Typ.Procname.Java curr_pname_java, Typ.Procname.Java pn_java
     -> if String.equal (Typ.Procname.java_get_class_name curr_pname_java)
  ...
  let typecheck_expr_simple typestate1 exp1 typ1 origin1 loc1 =
    typecheck_expr find_canonical_duplicate calls_this checks tenv node instr_ref curr_pdesc
      typestate1 exp1
      (typ1, TypeAnnotation.const AnnotatedSignature.Nullable false origin1, [loc1]) loc1
  in
  ...
  | Sil.Store (e1, typ, e2, loc)
   -> typecheck_expr_for_errors typestate e1 loc ;
      ...
      let typestate2 =
        match e1' with
        | Exp.Lvar pvar
         -> TypeState.add pvar (typecheck_expr_simple typestate1 e2 typ TypeOrigin.Undef loc)
              typestate1
        | Exp.Lfield _
         -> typestate1
        | _
  ...
  | Sil.Call (Some (id, _), Exp.Const Const.Cfun pn, [(array_exp, t)], loc, _)
   ...
   -> let _, ta, _ =
        typecheck_expr find_canonical_duplicate calls_this checks tenv node instr_ref curr_pdesc
          typestate array_exp
          (t, TypeAnnotation.const AnnotatedSignature.Nullable false TypeOrigin.ONone, [loc]) loc
      in
      if checks.eradicate then
        EradicateChecks.check_array_access tenv find_canonical_duplicate curr_pdesc node instr_ref
          array_exp (Typ.Fieldname.Java.from_string "length") ta loc false ;
      TypeState.add_id id
         ...
         -> let formals =
            ...
            let proc_attributes =
              { (ProcAttributes.default callee_pname Config.Java) with
                ProcAttributes.formals= formals; ret_type }
            in
            proc_attributes
      in
              ...
              ( if checks.eradicate && should_report then
                  ...
                  EradicateChecks.report_error tenv find_canonical_duplicate
                    (TypeErr.Condition_redundant
                       (true, EradicateChecks.explain_expr tenv node cond, false)) (Some instr_ref)
                    loc curr_pdesc ) ;
              TypeState.add pvar
        ...
        let pname_get_from_pname_put pname_put =
          ...
          Typ.Procname.java_replace_parameters
            (Typ.Procname.java_replace_return_type
               (Typ.Procname.java_replace_method pname_put "get") object_t)
            parameters
        in
        match call_params with
        ...
        let resolve_param i (sparam, cparam) =
          ...
          let _, ta2, _ =
            typecheck_expr find_canonical_duplicate calls_this checks tenv node instr_ref
              ...
              (t2, TypeAnnotation.const AnnotatedSignature.Nullable false TypeOrigin.ONone, []) loc
          in
        ...
        let typestate_after_call =
          if not is_anonymous_inner_class_constructor then (
            ...
            if Models.is_check_not_null callee_pname then
              do_preconditions_check_not_null (Models.get_check_not_null_parameter callee_pname)
          ...
          let map_dexp = function
            | Some
             ...
             -> let pname_java' =
                  let object_t = (Some "java.lang", "Object") in
                  Typ.Procname.java_replace_return_type
                    (Typ.Procname.java_replace_method pname_java "get") object_t
                in
