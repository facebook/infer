...
let debug () =
  if not Config.(global_tenv || procedures || source_files || Option.is_some export_textual) then
    ...
    ( if Config.procedures then
        ...
        let summary_of proc_name = Summary.OnDisk.get AnalysisRequest.all proc_name in
        ...
        if Config.procedures_summary || Config.procedures_summary_json then
          ...
          let json_of_summary proc_name =
            Summary.OnDisk.get AnalysisRequest.all proc_name |> Option.map ~f:Summary.yojson_of_t
          in
                  ...
                  PulseSpecializedCallGraph.JsonBuilder.add builder proc_name
                    (let open IOption.Let_syntax in
                     let* (summary : Summary.t) =
                       Summary.OnDisk.get AnalysisRequest.all proc_name
                     in
