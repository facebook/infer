...
and tree_sitter_preprocessor =
  CLOpt.mk_string_opt ~long:"tree-sitter-preprocessor" ~meta:"command"
    "Preprocessor command to run on C files before tree-sitter parsing. The command receives the \
     source file as argument and should write preprocessed output to stdout. Example: \
     $(b,--tree-sitter-preprocessor 'cc -E') or $(b,--tree-sitter-preprocessor 'clang -E \
     -Iinclude'). When not set, tree-sitter parses the original source directly."


and capture_rust_ullbc =
  CLOpt.mk_path_list ~long:"capture-rust-ullbc" ~meta:"path"
    "Generate a SIL program from a Rust ullbc file generated by charon."
...
and tree_sitter_preprocessor = !tree_sitter_preprocessor
