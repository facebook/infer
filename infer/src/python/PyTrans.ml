...
(** When a class doesn't define it's constructor [__init__] method, let's define a basic one
    instead. We need to do so because [__init__] might be called by a child class.

    This constructor will do nothing in general. If the class has a parent class, this constructor
    will call the parent constructor, as python's runtime is currently doing. *)
let default_init _env loc class_name parent =
  let typ = PyCommon.mk_type class_name in
  let formals_types = T.Typ.mk_without_attributes typ in
  let result_type = T.Typ.mk_without_attributes PyCommon.pyNone in
  let self = var_name ~loc PyCommon.self in
  let label = node_name ~loc PyCommon.entry in
  let process_parent_instrs parent =
    (* If there's a parent class, we want to call the parent constructor on [self] *)
    let self_id = T.Ident.of_int 0 in
    let load_self = T.Instr.Load {id= self_id; exp= T.Exp.Lvar self; typ; loc} in
    let id = T.Ident.next self_id in
    let super = Symbol.to_type_name ~is_static:false parent in
    let proc : T.qualified_procname =
      {T.enclosing_class= Enclosing super; name= proc_name ~loc PyCommon.init__}
    in
    let exp = T.Exp.Call {proc; args= [T.Exp.Var self_id]; kind= Virtual} in
    let call_super = T.Instr.Let {id; exp; loc} in
    [load_self; call_super]
  in
  let instrs = Option.value_map ~default:[] ~f:process_parent_instrs parent in
  let last = T.Terminator.Ret (T.Exp.Const T.Const.Null) in
  let node =
    {T.Node.label; ssa_parameters= []; exn_succs= []; last; instrs; last_loc= loc; label_loc= loc}
  in
  let enclosing_class = T.Enclosing (type_name ~loc class_name) in
  let qualified_name : T.qualified_procname =
    {T.enclosing_class; name= proc_name ~loc PyCommon.init__}
  in
  let procdecl =
    {T.ProcDecl.qualified_name; formals_types= Some [formals_types]; result_type; attributes= []}
  in
  let desc =
    {T.ProcDesc.procdecl; nodes= [node]; start= label; params= [self]; locals= []; exit_loc= loc}
  in
  T.Module.Proc desc
