...
let find_reason_to_skip_analysis proc_name proc_desc =
  match (proc_name, (Procdesc.get_attributes proc_desc).ProcAttributes.translation_unit) with
  | _, source when SourceFile.has_extension ~ext:Config.kotlin_source_extension source ->
      Some "Kotlin source file"
  | Procname.Java java_pname, _ ->
