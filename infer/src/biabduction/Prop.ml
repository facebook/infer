      ...
      let find_captured_variable_in_the_heap captured' hpred =
        ...
        | Hpointsto (Exp.Lvar var, Eexp (Exp.Var id, _), _) ->
            IList.map_changed ~equal:phys_equal
              ~f:(fun ((e_captured, (captured_var : CapturedVar.t)) as captured_item) ->
                match e_captured with
                | Exp.Var id_captured ->
                    if Ident.equal id id_captured && Pvar.equal var captured_var.pvar then
                      captured_item
                    else if Ident.equal id id_captured then
                      (e_captured, {captured_var with pvar= var})
                    else if Pvar.equal var captured_var.pvar then (Exp.Var id, captured_var)
                    else captured_item
                | _ ->
                    captured_item )
              captured'
...
let rec exp_captured_ren ren (e : Exp.t) : Exp.t =
  ...
  | Closure {name; captured_vars} ->
      let captured_vars' =
        List.map ~f:(fun (e, captured_var) -> (exp_captured_ren ren e, captured_var)) captured_vars
      in
