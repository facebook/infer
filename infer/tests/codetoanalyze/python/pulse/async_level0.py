...
async def FP_call_sleep_with_temp_ok():
    temp = sleep(1)
    await temp


async def FP_call_sleep_with_branchs_ok(b):
    temp = sleep(1)
    if b:
        await temp
    else:
...
asyncio.run(FP_call_sleep_with_temp_ok())


asyncio.run(FP_call_sleep_with_branchs_ok(b))
