...
int* funptr_conditional_call(int x, int* p) {
  int* ptr = p;
  void (*funptr)(int**);
  if (!x) {
    funptr = &assign_NULL;
...
int funptr_conditional_call_bad(int* p) {
  int* q = funptr_conditional_call(0, p);
  return *q;
}

int funptr_conditional_call_good(int* p) {
  int* q = funptr_conditional_call(1, p);
  return *q;
}
