...
// Pins frontend handling of `weak var x: AnyObject?`.  The Swift compiler
// emits `swift_unknownObjectWeakLoadStrong` (the ObjC-aware variant of
// `swift_weakLoadStrong`) for the autogenerated `delegate.get` accessor,
// and that helper sits in `Llair2TextualModels.functions_to_skip` because
// the IR has already lifted the slot read to an explicit `Load` before
// the call.  The captured SIL must show the skip-to-first-arg form
// rather than a call to the helper — see the `n5 = n4` line inside
// `WeakDelegateHolder.delegate.get` in `rc_delegate.swift.sil`; without
// the one-line addition it would read `n5 = swift_unknownObjectWeakLoadStrong(n4)`.
class WeakDelegateHolder {
    weak var delegate: AnyObject?
}

func read_weak_delegate(h: WeakDelegateHolder) -> AnyObject? {
    return h.delegate
}
