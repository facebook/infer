...
(** A tabulation-based solver to compute distance heuristics along some goal
    trace of procedure calls and returns. *)
module Distance_tabulation = struct
  (** An automaton state in the sequence of calls and returns that the
      symbolic executor will attempt to follow. Represented as an integer
      and interpreted as an amount of progress through the shared ref
      [trace], analogously to the [cursor] of [Goal.Sparse_trace] but with a
  ...
  module Automaton_state = struct
    ...
    (** Because there are always exactly [IArray.length !trace + 1]
        automaton states, we can represent a "table" keyed on automaton
        states using an array of that length. *)
    module Tbl = struct
      type 'a t = 'a option array

      let create () =
        Array.init (IArray.length !trace + 1) ~f:(fun _ -> None)
  ...
  module Distance_map = struct
    ...
    (** A distance map [{max; offsets}] conceptually maps each key [k] of
        ...
        manipulate than a direct representation. Note that this
        representation is non-canonical: two different values can represent
        the same distance map, if the difference in their [max] field is
        equal to the pointwise difference between each of their offsets.
        This requires [equal] below to do some extra work, but no additional
    ...
    let join {max; offsets} {max= max'; offsets= offsets'} =
      (* When [max = max'], the offsets can be joined directly with
         pointwise `Int.min`. When they are unequal, we take the larger one
         and shift the offsets of the other distance map accordingly. *)
      if max = max' then
        { max
        ; offsets=
            Map.union offsets offsets' ~f:(fun _ d d' ->
                Some (Int.max d d') ) }
  ...
  (** A [summary] is composed of
      ...
      (1) the length of the shortest path from procedure entry to the
      current location,
      ...
      (4) the lengths of the shortest paths to each trace checkpoint that
      has already been reached from the program entry. *)
  module Summary = struct
    ...
    (* Machinery to deal with pausing/restarting _intraprocedural_ analyses
       ...
       (mutable) invariant map and worklist with each intraprocedural
       analysis problem (indexed by function name and initial automaton
       state). *)
    type intraproc_work =
    ...
    (** Interprocedural worklist, consisting of two types of work:
        "Summarize" work elements require computing a summary of some
        function in some initial automaton state, while "Fix" elements
        require computing the fixed point of some summary of [caller_name]
        with a recursive call to [callee_name]. *)
    ...
    (** Compute and return a summary of the given [func] in the given
        [init_state]. Raises a [Summary_query] if a callee summary is
        required that has not yet been computed. *)
            ...
            | Call {callee; return; _} -> (
                ...
                let call {func= callee; _} =
                  let is_recursive () =
                    ...
                    in
                    let mutually () =
                      Worklist.contains callee.name state.auto_state
                        worklist
                    in
                    directly || mutually ()
                  in
      ...
      while not (Stack.is_empty worklist) do
        ...
        | Fix
            ...
            (* If there has been no change in the summary for which a fixed
               ...
               corresponding `Summarize`, so we compare the result summary
               of that `Summarize` to the original (possibly unsound)
               summary that was applied to compute it.

               Otherwise, propagate any new dataflow to each recursive
               return site in the intraprocedural analysis for this summary
               and continue. *)
            if Option.exists curr_summary ~f:(equal prev_summary) then ()
              ...
              Func.fold_cfg caller () ~f:(fun blk () ->
                  ...
                  | Call {callee= Indirect {candidates; _}; _} ->
                      if IArray.exists candidates ~f:is_recursive_callee
                      then resummarize_call ()
                  | _ -> () )
        | Summarize (f, a) -> (
          match lookup_summary f a with
          | Some _ ->
              ()
              (* this summary has already been computed on some other
                 path *)
          | None -> (
            try
              summarize
