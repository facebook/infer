...
/* llvalue -> string option */
value llvm_get_debug_loc_directory(LLVMValueRef Val) {
  CAMLparam0();
  CAMLlocal1(Option);
  unsigned Length = 0;
  const char *Chars;
...
/* llvalue -> string option */
value llvm_get_debug_loc_filename(LLVMValueRef Val) {
  CAMLparam0();
  CAMLlocal1(Option);
  unsigned Length = 0;
  const char *Chars;
...
/* llvalue -> int */
value llvm_get_debug_loc_line(LLVMValueRef Val) {
  unsigned L = 0;
  if (LLVMIsAInstruction(Val)) {
    LLVMMetadataRef Loc = LLVMInstructionGetDebugLoc(Val);
    if (Loc) {
...
/* llvalue -> int */
value llvm_get_debug_loc_column(LLVMValueRef Val) {
  unsigned C = 0;
  if (LLVMIsAInstruction(Val)) {
    LLVMMetadataRef Loc = LLVMInstructionGetDebugLoc(Val);
    if (Loc) {
